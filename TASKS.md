...
- [ ] **1.1** Initialise the Vite + React + TypeScript project in the repo root:
  ```
  npm create vite@latest . -- --template react-ts
  ```
  Accept prompts to scaffold into the current directory.

- [ ] **1.2** Install all runtime and dev dependencies:
  ```
  npm install d3 zustand zundo
  npm install -D @types/d3 tailwindcss @tailwindcss/vite
  ```

- [ ] **1.3** Configure Tailwind: add the `@tailwindcss/vite` plugin to `vite.config.ts`
...
- [ ] **1.4** Set `base: '/Zones/'` in `vite.config.ts` for GitHub Pages deployment.

- [ ] **1.5** Source the FRC 2026 field image and place it at `public/field2026.png`.
...
- [ ] **1.6** Create `public/fieldmap2026.json` with snap points derived from the
...
- [ ] **1.7** Delete the default Vite boilerplate files: `src/App.css`,
...
- [ ] **2.3** Create `src/history.ts`: export `useUndo`, `useRedo`, `useCanUndo`,
  `useCanRedo` hooks built on top of `useTemporalStore` from `zundo`.

- [ ] **2.4** In `src/App.tsx`, add a `useEffect` that fetches `fieldmap2026.json`
...
- [ ] **3.1** Create `src/components/FieldCanvas.tsx`:
  - Outer `<div>` fills remaining viewport space (flex-1).
  - Inner `<svg>` with `viewBox="0 0 17.548 8.052"` and `preserveAspectRatio="xMidYMid meet"`.
  - `<image>` element renders `field2026.png` as the background.
  - Attach `d3-zoom` to the SVG for pan and scroll-to-zoom (scale 0.5–20×).
  - Expose the current D3 transform via a `FieldTransformContext` React context
...
- [ ] **3.2** Create `src/components/ZonePolygon.tsx`:
  - Renders one `Zone` as an SVG `<polygon>` (fill with zone color at 30% opacity,
    stroke at full opacity) and a `<circle r="0.15">` at each vertex.
  ...
  - Double-click on the polygon fill (not a vertex) inserts a new point at the
    position on the nearest edge (find closest edge, insert at that index).
  - Click on the polygon fill calls `store.selectZone()`.
  - Selected zone polygon gets a thicker stroke and vertex circles get a highlight ring.

- [ ] **3.3** Create `src/components/SnapOverlay.tsx`:
...
- [ ] **3.4** In `FieldCanvas.tsx`, render all visible zones as `<ZonePolygon>`
...
- [ ] **4.1** Create `src/components/ZoneSidebar.tsx`:
...
- [ ] **4.2** Create `src/components/Toolbar.tsx`:
...
- [ ] **5.1** Create `src/components/ExportButton.tsx`:
...
- [ ] **5.2** Create `src/components/ImportButton.tsx`:
...
- [ ] **6.1** In `src/App.tsx`, add a `useEffect` that attaches a `keydown` listener
  to `window`:
  - `Ctrl+Z` → `undo()`
...
- [ ] **6.2** Wire up `src/App.tsx` layout: `<Toolbar>` at top, `<ZoneSidebar>` on
...
- [ ] **6.4** Run `npm run build` and confirm zero TypeScript errors and a clean
