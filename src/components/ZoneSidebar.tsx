        ...
        {zones.map((zone, index) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col gap-1 px-3 py-2 border-b border-[#2a2a2a] cursor-pointer select-none transition-colors ${
                isSelected ? 'bg-[#1a1a1a]' : 'hover:bg-[#1a1a1a]'
              } ${isDragTarget ? 'border-t-2 border-t-[#FFCC00]' : ''}`}
              onClick={() => selectZone(zone.id)}
            >
              {/* Row 1: grip, color, name, eye, trash */}
              <div className="flex items-center gap-2">
                {/* Drag handle */}
                <span
                  className="text-[#555] hover:text-[#888] cursor-grab shrink-0 text-sm leading-none"
